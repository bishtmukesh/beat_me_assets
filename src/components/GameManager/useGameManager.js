...
const useGameManager = ( { roomCode, stompClient, setGameUpdateHandler } ) => {
    ...
    const sendPictionaryUpdateMessage = useCallback((updateType, point) => {
        if (stompClient && stompClient.connected) {
            if (updateType === DRAWING_UPDATE_TYPES.ADD_TO_SEGMENT) {
                stompClient.send(PICTIONARY_UPDATE_MESSAGE_ENDPOINT, {}, JSON.stringify({ roomCode, updateType, point }));
            } else {
